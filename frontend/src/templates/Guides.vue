        ...
        <section class="main">
            <div class="container">
                ...
                >
                    ...
                    />
                    <a-doc :key="$route.path" :doc="$context.doc" style="flex: 1 auto"/>
                    <template v-if="$context.toc">
                        <a-toc :key="$route.path + 'toc'" :node="$context.toc" class="toc-wrapper"/>
                    </template>
...
<script>
    ...
    export default {
        components: { AHeader, ASidebar, ADoc, AToc, AFooter },
        ...
        metaInfo() {
            return {
                ...
                meta: [
                    {
                        name: 'description',
                        content: 'TNGraphQL is a fully-featured MVC framework for Node.js. It takes care of most of your web development hassles, offering you a clean and stable API to build web apps or microservices.'
                    },
                    { name: 'keywords', content: 'tngraphql, graphql, framework, node.js, mvc, javascript' },
                    { property: 'og:title', content: `${ this.$context.doc.title } - TNGraphQL Framework` },
                    {
