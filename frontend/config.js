...
exports.header = [
  ...
  },
  {
    ...
    children: [
      {
        title: 'Github',
        permalink: 'https://github.com/tngraphql',
        isExternal: true
      },
      {
        title: 'Facebook',
        permalink: 'https://www.facebook.com/groups/2585371045115513',
        isExternal: true
      }
    ]
  }
...
exports.showCaseTabs = [
  {
    title: 'routers',
    code: `<pre><code><span class="token keyword">import</span> <span class="token punctuation">{</span> Route <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@tngraphql/illuminate/dist/Support/Facades'</span><span class="token punctuation">;</span>

Route<span class="token punctuation">.</span><span class="token function">query</span><span class="token punctuation">(</span><span class="token string">'posts'</span><span class="token punctuation">,</span> <span class="token strin
g">'PostResolve.index'</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
Route<span class="token punctuation">.</span><span class="token function">mutation</span><span class="token punctuation">(</span><span class="token string">'createPost'</span><span class="token punctuation">,</span> <span class="tok
en string">'PostResolve.store'</span><span class="token punctuation">)</span>
  </code></pre>`
  },
  {
    title: 'Resolver',
    code: `<pre><code>@<span class="token function">Resolver</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
<span class="token keyword">export</span> <span class="token keyword">class</span> <span class="token class-name">PostResolve</span> <span class="token keyword">extends</span> <span class="token class-name">BaseResolve</span> <span
class="token punctuation">{</span>
    @<span class="token function">Query</span><span class="token punctuation">(</span><span class="token parameter">returns</span> <span class="token operator">=></span> <span class="token punctuation">[</span>UserType<span class="t
oken punctuation">]</span><span class="token punctuation">)</span>
    ...
    @<span class="token function">Mutation</span><span class="token punctuation">(</span><span class="token parameter">returns</span> <span class="token operator">=></span> UserType<span class="token punctuation">)</span>
    <span class="token keyword">async</span> <span class="token function">store</span><span class="token punctuation">(</span><span class="token parameter">@<span class="token function">Args</span><span class="token punctuation">(</span><span class="token punctuation">)</span> data<span class="token operator">:</span> PostArgs</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">const</span> post <span class="token operator">=</span> <span class="token keyword">await</span> Post<span class="token punctuation">.</span><span class="token function">create</span><span class="
token punctuation">(</span>data<span class="token punctuation">)</span>
...
<span class="token punctuation">}</span>
  ...
  {
    title: 'Models',
    code: `<pre><code><span class="token keyword">import</span> <span class="token punctuation">{</span> column <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@tngraphql/lucid/build/src/Orm/Decorators'</span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token punctuation">{</span> BaseModel <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@tngraphql/lucid/build/src/Orm/BaseModel'</span><span class="token punctuation">;</span>

<span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token keyword">class</span> <span class="token class-name">Post</span> <span class="token keyword">extends</span> <span class="token c
lass-name">BaseModel</span> <span class="token punctuation">{</span>
  @<span class="token function">column</span><span class="token punctuation">(</span><span class="token punctuation">{</span> isPrimary<span class="token operator">:</span> <span class="token boolean">true</span> <span class="token
...
exports.primaryFeatures = [
  ...
  },
  {
    ...
    body: `Stop spending hours to find the perfect NPM packages. TNGraphQL comes with a ton of first party packages with unified API semantics.`
  }
]
    ...
    features: [
      {
        ...
        body: `TNGraphQL ships with one of the fastest HTTP router without compromising its features set. Features like Route groups, Subdomains, Resourceful routes are all baked in.`,
      },
      {
        ...
        body: `Being a TypeScript first framework, TNGraphQL exposes the API to define runtime validations on the request body and also extract the static type information at the same time.`,
      },
      {
      ...
      },
      {
        ...
        body: `TNGraphQL is the only Node.js framework that has first class support for JSON:API. If your team has settled on using JSON:API, then adding it to backend is just a matter of few minutes.`,
      }
    ],
  },
  {
    ...
    features: [
      {
        ...
        body: `TNGraphQL is one of the few Node.js frameworks to treat SQL as the first class citizen. We support all the main stream SQL servers like PostgreSQL, MySQL, MSSQL, MariaDB and many more.`,
      },
      {
        ...
        body: `Inspired by Rails and Laravel, TNGraphQL ships with an implementation of Active Record ORM. The ORM offers a rich API for running complex SQL queries and managing relationships.`,
      },
      {
        title: 'Migrations',
        ...
        body: `TNGraphQL has in-built support for database migrations. As your application evolves, you can <strong>create</strong> and <strong>alter</strong> the database schema using the JavaScript code.`,
      },
      {
    ...
    features: [
      {
        ...
        body: `There is no one size fit authentication process for all apps and hence TNGraphQL has let you choose between Sessions, Opaque tokens and JWT tokens.`,
      },
      {
        title: 'InBuilt RBAC',
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        body: `THe shield package of TNGraphQL comes with a suite of guards to protect your applications from common attacks like <strong>XSS</strong>, <strong>clickjacking</strong>, <strong>script injection</strong> and many more.`,
      },
      {
        title: 'CORS Enabled',
        icon: `<svg class="feature-icon" width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg"><g fill="none" fill-rule="evenodd"><path d="M0 0h40v40H0z"/><g stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"><path stroke="#5A45FF" d="M13.75 20h12.5v8.75h-12.5zM16.25 20v-3.75A3.75 3.75 0 0120 12.5h0a3.75 3.75 0 013.75 3.75V20"/><path d="M35 23.75c0 8.284-6.716 15-15 15-8.284 0-15-6.716-15-15V6.25L20 2.5l15 3.75v17.5z" stroke="#17161A"/></g></g></svg>`,
        body: `TNGraphQL let you manage the security settings for <strong>Cross Origin HTTP Requests</strong> to make sure that only authorized and valid requests are making their way through the application.`,
      }
    ],
  },
  {
    ...
    features: [
      {
        ...
        body: `In the era of containers and orchestration, health checks plays an important role to effectively manage resources. TNGraphQL comes with handful of in-built health checks.`,
      },
      {
